...
class SubGridGlobalBasisRestrictor
{
public:
  ...
  /** \brief Restrict host grid function to subgrid
  *
  * This is a shortcut for creating an (temporary) restrictor
  * object and handing it to the transfer method in the subgrid.
  */
  static void restrict(const GridType& _grid,
      const Vector& _hostGridFunction,
      Vector& _subGridFunction)
  {
    typedef SubGridGlobalBasisRestrictor<GridType, Vector> MyType;
  ...
  SubGridGlobalBasisRestrictor(const GridType& _grid,
      const Vector& _hostGridFunction,
      Vector& _subGridFunction)
    : grid(_grid),
