...
class LagrangeDGBasisLocalView;

template<typename GV, int k>
class LagrangeDGBasisLeafNode;

template<typename GV, int k>
class LagrangeDGIndexSet;
...
public:
  ...
  typedef LagrangeDGBasisLocalView<GV,k> LocalView;

  /** \brief Type used for global numbering of the basis vectors */
  typedef std::array<size_type, 1> MultiIndex;

  LagrangeDGLocalIndexSet(const LagrangeDGIndexSet<GV,k> & indexSet)
   ...
   */
  void bind(const LagrangeDGBasisLocalView<GV,k>& localView)
  {
  ...
  const LagrangeDGBasisLocalView<GV,k>* localView_;

  const LagrangeDGIndexSet<GV,k> basisIndexSet_;
};
...
class LagrangeDGIndexSet
{
  ...
  // Needs the mapper
  friend class LagrangeDGLocalIndexSet<GV,k>;

  // Precompute the number of dofs per entity type
  const int dofsPerEdge        = k+1;
  const int dofsPerTriangle    = ((k+1)*(k+2))/2;
  const int dofsPerQuad        = (k+1)*(k+1);
  const int dofsPerTetrahedron = (k+1)*(k+2)*(k+3)/6;
  const int dofsPerPrism       = ((k+1)*(k+1)*(k+2))/2;
  const int dofsPerHexahedron  = (k+1)*(k+1)*(k+1);
  const int dofsPerPyramid     = ((k+1)*(k+2)*(2*k+3))/6;

public:

  typedef LagrangeDGLocalIndexSet<GV,k> LocalIndexSet;

  LagrangeDGIndexSet(const GV& gridView)
  : gridView_(gridView)
  {
    switch (dim)
    {
 ...
 */
template<typename GV, int k>
class LagrangeDGBasis
: public GridViewFunctionSpaceBasis<GV,
                                    LagrangeDGBasisLocalView<GV,k>,
                                    LagrangeDGIndexSet<GV,k>,
...
public:
  ...
  LagrangeDGBasis(const GridView& gv) :
    gridView_(gv),
    indexSet_(gv)
  {}
  ...
  LagrangeDGIndexSet<GV,k> indexSet() const
  {
    return indexSet_;
  }
...
protected:
  ...
  LagrangeDGIndexSet<GV,k> indexSet_;
};
...
public:
  ...
  typedef LagrangeDGBasis<GV,k> GlobalBasis;
   ...
   */
  typedef LagrangeDGBasisLeafNode<GV,k> Tree;

  /** \brief Construct local view for a given global finite element basis */
  LagrangeDGBasisLocalView(const GlobalBasis* globalBasis) :
   ...
   * And indeed, in the LagrangeDGBasisView implementation this method does nothing.
   */
  void unbind()
  {}
   ...
   *
   * The method returns (k+1)^dim, which is the number of degrees of freedom you get for cubes.
   */
...
class LagrangeDGBasisLeafNode :
  public GridFunctionSpaceBasisLeafNodeInterface<
    ...
    typename LagrangeDGBasis<GV,k>::size_type>
{
  typedef LagrangeDGBasis<GV,k> GlobalBasis;
  static const int dim = GV::dimension;
...
public:
  ...
  LagrangeDGBasisLeafNode(const GlobalBasis* globalBasis) :
    globalBasis_(globalBasis),
